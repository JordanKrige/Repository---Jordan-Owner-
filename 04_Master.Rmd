...
>*Answer 1: Possible explanations for why NDVI saturates with time since fire could include senescence, increased competition with other plants that are growing and therefore requiring more resources and that fynbos needs fire to reseed/resprout and so a lack of fire for a number of years means there are no new plants so NDVI stabilizes.*
...
>*Answer 2: There are three common parameters between model 1 and 2, namely alpha, gamma and lambda. In the first model, a one unit increase in alpha leads to a 0.25107 increase in NDVI, holding all other variables constant. This is a bigger change than that in the second model where a increase of one unit in alpha leads to a 0.207522 change in NDVI. Gamma in the first model has a smaller effect for a one unit change than gamma in model two. In model one, a one unit change in gamma leads to a 0.32371 change in NDVI holding all other variables constant, while a one unit change in gamma in model two leads to a 0.364746 change in NDVI, holding all other variables constant. Lastly, a one unit increase in lambda leads to a 1.17687 change in NDVI for model one, but only a 0.989154 change in NDVI for model two. *
...
>*Answer 3: Model 2 is better as it has a smaller residual sum of squares and has a smaller standard error. In addition to this, the p-value=4.12e-14 observed for the second model is significantly small.* 

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*Answer 4: To calculate degrees of freedom (df) you use the following formula, df = n-p-1, where n is the sample size and p is the number of parameters. There are 139 data entries, so n=139. Model 1 has 3 parameters so df = 139-3-1 = 135. In model 2 df = 139-5-1 = 133 since there are 5 parameters in model 2.  *
...
>*Answer 5: The full model has a lower AIC of 451155.3 and is therefore preferred. This is supported by the difference between the two AIC's being far larger than 3, implying that the models are significantly different at an p-value < 0.05*
...
>*Answer 6: MLE models maximize the log-likelihood, whereas NLS does not maximize log-likelihood. And for this reason, their AIC's should not be compared.  *
